...
const formSchema = z.object({
  url: z.string().min(1, {
    message: "A file url is required.",
  }),
});
...
const AttachmentsForm = ({ initialData, courseId }: AttachmentsFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.error(e.message ?? "failed to update attachments form");
    }
  };
  ...
  const onDelete = async (id: string) => {
    try {
      ...
      toast.success("Attachment was deleted successfully.");
      router.refresh();
    } catch (e: any) {
      ...
      {isEditing && (
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-4">
            Add anything your students might need to download to complete the
            course.
          </div>
        </div>
      )}
...
// WIP - fixing delete bug -  https://youtu.be/Big_aFLmekI?t=14555
